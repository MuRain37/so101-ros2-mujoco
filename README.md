...
四段展示的最终状态均满足对应任务的成功条件：分类任务要求两个方块进入同色目标
区域，抽屉任务要求打开抽屉、放入红色方块并重新关闭。两段 SmolVLA 展示均为一次
连续推理回合，分别在 20 秒内完成任务；成片只裁掉复位等待和完成后的停顿，没有删除
中间动作。这些展示用于定性说明系统能力，不代表统计成功率。展示使用固定随机种子
复现：分类任务 ACT 为 42、SmolVLA 为 21，抽屉任务均为 11。
