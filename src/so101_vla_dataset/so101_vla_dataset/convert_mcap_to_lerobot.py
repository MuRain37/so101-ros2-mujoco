...
def joint_positions(message):
    ...
    missing = [joint for joint in JOINTS if joint not in by_name]
    if missing: raise ValueError("joint message is missing: " + ", ".join(missing))
    return np.asarray([by_name[joint] for joint in JOINTS], dtype=np.float32)

def convert_episode(dataset, episode_dir, task_id, tolerance):
    ...
    for message in front:
        ...
        dataset.add_frame({"observation.images.front": image_array(message),
                           ...
                           "observation.state": joint_positions(state_msg),
                           "action": joint_positions(action_msg), "task": task})
        count += 1
...
def main(argv=None):
    ...
    features = {
        ...
        "observation.state": {"dtype": "float32", "shape": (6,), "names": {"axes": list(JOINTS)}},
